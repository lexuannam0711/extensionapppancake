...
test('server boundary: review queue API accepts protected reads', async () => {
  const server = createApp({ authToken: 'local-secret' }).listen(0, '127.0.0.1');
  await new Promise((resolve) => server.once('listening', resolve));
  try {
    const address = server.address();
    const response = await fetch(`http://127.0.0.1:${address.port}/api/review-queue`, {
      headers: { 'X-Local-Api-Token': 'local-secret' }
    });
    assert.equal(response.status, 200);
    assert.deepEqual((await response.json()).items instanceof Array, true);
  } finally {
    await new Promise((resolve) => server.close(resolve));
  }
});
